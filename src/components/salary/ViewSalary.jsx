...
const ViewSalary=()=>{
    ...
    useEffect(() => {
        const fetchSalaries = async () => {
            try {
                const response = await axios.get(`https://employee-salary-leave-api-bazz.vercel.app/api/salary/${id}/${user.role}`, {
                    headers: {
        ...
        <div className="overflow-x-auto p-5">
            ...
            {filteredSalaries.length > 0 ?(
                <table className="w-full text-sm text-left text-gray-500">
                    <thead className="text-x5 text-gray-700 uppercase bg-gray-50 border border-gray-200">
                        <tr>
                        ...
                        {filteredSalaries.map((salary) => (
                            <tr key={salary.id}
                              className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td className="px-6 py-3">{salary.allowences}</td>
