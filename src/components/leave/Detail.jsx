...
const Detail = ()=>{
    ...
    const changeStatus=async(id,status)=>{
         try {
                const response = await axios.put(`https://employee-salary-leave-api-bazz.vercel.app/api/leave/${id}`, {status}, {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem('token')}`
                    },
                });
                    ...
                    <p className="text-lg font-bold">{leave.status=="Pending"?"Action:":"Status"}</p>
                    {
                        leave.status=="Pending"?(
                            <div className="flex space-x-2">
                                ...
                                onClick={()=> changeStatus(leave._id,"Approved")}>Approave</button>
